xmlport 50102 "EI-ExportCreditMemo"
{
    Format = Xml;
    Direction = Export;
    Encoding = UTF8;
    schema
    {
        ...
        textelement(FACTURA)
        {
            tableelement(ENC; "Sales Cr.Memo Header")
            {
                textelement(ENC_1)
                {
                        ...
                        case "NOT".number of
                            ...
                            6:
                                begin
                                    ...
                                    NOT_1 := format(ENC."Amount Including VAT");
                                end;
            ...
            tableelement("ITE"; "Sales Cr.Memo Line")
            {
                LinkTable = ENC;
    ...
    var
        crMemoLines: Record "Sales Cr.Memo Line";
    begin
        rtNoOfLines := 0;

        Clear(crMemoLines);
        crMemoLines.SetRange("Document No.", pDocNo);
        crMemoLines.SetFilter(Quantity, '<>%1', 0);
        rtNoOfLines := crMemoLines.Count;
    end;
    ...
    begin
        ...
        crMemoLines.SetRange("Document No.", pDocNo);
        crMemoLines.SetFilter(Quantity, '<>%1', 0);
        if crMemoLines.FindSet then begin
            amountExclVAT += crMemoLines.GetLineAmountExclVAT();
        end;
    ...
    begin
        ...
        crMemoLines.SetFilter(Quantity, '<>%1', 0);
        if crMemoLines.FindSet then begin
            baseVAT += crMemoLines."VAT Base Amount";
        end;
        rtBaseVAT := baseVAT;
    end;
    ...
    begin
        ...
        crMemoLines.SetFilter(Quantity, '<>%1', 0);
        if crMemoLines.FindSet then begin
            amountInclVAT += crMemoLines."Amount Including VAT";
        end;
    ...
    begin
        ...
        crMemoLines.SetFilter(Quantity, '<>%1', 0);
        if crMemoLines.FindSet then begin
            discountAmount += crMemoLines."Line Discount Amount";
        end;
