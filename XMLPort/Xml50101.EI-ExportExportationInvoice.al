xmlport 50101 "EI-ExportExportationInvoice"
{
    Format = Xml;
    Direction = Export;
    Encoding = UTF8;
        ...
        textelement(FACTURA)
        {
            tableelement(ENC; "Sales Invoice Header")
            {
                ...
                begin
                    ...
                    TransportMethod.Get(ENC."Transport Method");

                    Clear(PaymentTerms);
                    PaymentTerms.Get(ENC."Payment Terms Code");
                end;
                ...
                textelement(NOT_1)
                {
                    ...
                    begin
                        case "NOT".number of
                            ...
                            3:
                                begin
                                    NOT_1 := TransportMethod.Description;
                                end;

                            4:
                                begin
                                    NOT_1 := PaymentTerms.Description;
                                end;

                            5:
                                begin
                                    NOT_1 := ENC."Shiptment Port";
                                end;

                            6:
                                begin
                                    NOT_1 := ENC."Gross Weight";
                                end;

                            7:
                                begin
                                    NOT_1 := ENC."Shipping Information";
                                end;

                            8:
                                begin
                                    NOT_1 := ENC."Destination Port";
                                end;

                            9:
                                begin
                                    NOT_1 := ENC."Net Weight";
                                end;

                            10:
                                begin
                                    NOT_1 := EISetup."Export Observations";
                                end;

                            11:
                                begin
                                    NOT_1 := EISetup."Export Expense 1";
                                end;

                            12:
                                begin
                                    NOT_1 := EISetup."Export Expense 2";
                                end;

                            13:
                                begin
                                    NOT_1 := 'TOTAL' + ENC."Shiptment Port";
                                end;
                            ...
                            18:
                                begin
                                    NOT_1 := format(AmountInclVatFreight(ENC."No."));
                                end;

                            19:
                                begin
                                    NOT_1 := format(AmountInclVatOtherExpenses(ENC."No."));
                                end;

                            20:
                                begin
                                    NOT_1 := format(AmountInclVatFreight(ENC."No.") + AmountInclVatOtherExpenses(ENC."No.") + AmountInclVatWithoutMarks(ENC."No."));
                                end;

                            21:
                                begin
                                    NOT_1 := format(AmountInclVatInternationalFreight(ENC."No."));
                                end;

                            22:
                                begin
                                    NOT_1 := format(AmountInclVatInsurance(ENC."No."));
                                end;
    ...
    local procedure AmountInclVatFreight(pDocNo: Code[20]) rtAmount: Decimal;
    var
        invoiceLines: Record "Sales Invoice Line";
        amount: Decimal;
    begin
        ...
        Clear(invoiceLines);
        invoiceLines.SetRange("Document No.", pDocNo);
        invoiceLines.SetRange(Freight, true);
        if invoiceLines.FindSet then begin
            repeat
                amount += invoiceLines."Amount Including VAT";
    ...
    local procedure AmountInclVatOtherExpenses(pDocNo: Code[20]) rtAmount: Decimal;
    var
        invoiceLines: Record "Sales Invoice Line";
        amount: Decimal;
    begin
        ...
        Clear(invoiceLines);
        invoiceLines.SetRange("Document No.", pDocNo);
        invoiceLines.SetRange("Other Expenses", true);
        if invoiceLines.FindSet then begin
            repeat
                amount += invoiceLines."Amount Including VAT";
            until invoiceLines.Next = 0;
    ...
    local procedure AmountInclVatInternationalFreight(pDocNo: Code[20]) rtAmount: Decimal;
    var
        invoiceLines: Record "Sales Invoice Line";
        amount: Decimal;
    begin
        ...
        Clear(invoiceLines);
        invoiceLines.SetRange("Document No.", pDocNo);
        invoiceLines.SetRange("International Freight", true);
        if invoiceLines.FindSet then begin
            repeat
                amount += invoiceLines."Amount Including VAT";
            until invoiceLines.Next = 0;
    ...
    local procedure AmountInclVatInsurance(pDocNo: Code[20]) rtAmount: Decimal;
    var
        invoiceLines: Record "Sales Invoice Line";
        amount: Decimal;
    begin
        ...
        Clear(invoiceLines);
        invoiceLines.SetRange("Document No.", pDocNo);
        invoiceLines.SetRange(Insurance, true);
        if invoiceLines.FindSet then begin
            repeat
                amount += invoiceLines."Amount Including VAT";
            until invoiceLines.Next = 0;
    ...
    local procedure AmountInclVatWithoutMarks(pDocNo: Code[20]) rtAmount: Decimal;
    var
        invoiceLines: Record "Sales Invoice Line";
        amount: Decimal;
    begin
        ...
        Clear(invoiceLines);
        invoiceLines.SetRange("Document No.", pDocNo);
        invoiceLines.SetRange(Freight, false);
        invoiceLines.SetRange("Other Expenses", false);
        invoiceLines.SetRange("International Freight", false);
        invoiceLines.SetRange(Insurance, false);
        if invoiceLines.FindSet then begin
            repeat
                amount += invoiceLines."Amount Including VAT";
            until invoiceLines.Next = 0;
